...
export default function InventoryTable({ tableData }) {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          const updatedInventory = inventory.filter(
            (item) => item.warehouseId !== warehouseId || item.gameId !== gameId
          );
  ...
  const renderDeleteButtons = (warehouseId, gameId) => {
    if (
      deleteConfirmationId.warehouseId === warehouseId &&
      deleteConfirmationId.gameId === gameId
    ) {
      return (
        <>
          ...
          <Button
            onClick={() => handleConfirmDelete(warehouseId, gameId)}
            secondary
          >
            Confirm
          </Button>
          ...
          {tableData.map((inventory, index) => {
            return (
              <tr key={index}>
                ...
                <td>
                  {renderDeleteButtons(inventory.warehouseId, inventory.gameId)}
                </td>
              </tr>
            );
          })}
